...
const WEIGHT_FILE_DIR = path.join(__dirname, 'output');
const MODEL_FILE_PATH = path.join(WEIGHT_FILE_DIR, 'model.json');
...
async function trainOrLoadModel(useTestData, callback) {
  if (fs.existsSync(MODEL_FILE_PATH)) {
    model = await tf.loadLayersModel(`file://${MODEL_FILE_PATH}`)
    callback('accuracyPerClass', await evaluate(useTestData));
    ...
    // save
    await model.save(`file://${WEIGHT_FILE_DIR}`);

    callback('trainingComplete', true);
